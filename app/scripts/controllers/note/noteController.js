angular.module('playerApp').controller('NoteListCtrl', function ($rootScope, noteService, config, $window, $timeout, $location, $stateParams) {

    var noteList = this;
    noteList.userId = $window.localStorage.getItem('user') ? JSON.parse($window.localStorage.getItem('user')).userId : $rootScope.userId;
    noteList.courseId = $stateParams.courseId === '0' ? '' : $stateParams.courseId;
    noteList.contentId = $stateParams.contentId === '0' ? '' : $stateParams.contentId;
    noteList.add = {};
    noteList.update = {};
