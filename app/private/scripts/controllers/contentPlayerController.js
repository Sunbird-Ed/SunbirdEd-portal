...
angular.module('playerApp')
    .controller('contentPlayerCtrl', function (playerTelemetryUtilsService,
        $state, $scope, $sce, contentService, pdfDelegate, $timeout,
        $stateParams, config, $rootScope) {
        ...
        function showPlayer(data) {
            ...
            window.onbeforeunload = function (e) { // eslint-disable-line
                playerTelemetryUtilsService
                .endTelemetry({ progress: $scope.contentProgress });
            };

            if ($scope.contentData.mimeType
                === 'application/vnd.ekstep.ecml-archive'
                || $scope.contentData.mimeType
                ...
                $timeout(function () {
                    ...
                    previewContentIframe.onload = function () {
                        ...
                        = org.sunbird.portal.channel;
                        if (_.isUndefined($stateParams.courseId)) {
                            configuration.context.dims
                            = org.sunbird.portal.dims;
                        } else {
                            var cloneDims
                            = _.cloneDeep(org.sunbird.portal.dims);
                        ...
                        .initializePreview(configuration);
                    };
                }, 1000);
            } else {
        ...
        $scope.next = function () {
            ...
            $scope.getCurrentPage
            = $scope.getCurrentPage < $scope.totalPageNumber
            ? $scope.getCurrentPage + 1 : $scope.getCurrentPage;
            ...
            var telemetryData = {
                ...
                subtype: '',
                data: {
                    stageId: $scope.getCurrentPage
                    ? $scope.getCurrentPage.toString() : ''
                }
            };
            ...
            var telemetryNavData = {
                ...
                ver: $scope._instance.ver,
                stageid: ($scope.getCurrentPage - 1).toString(),
                stageto: $scope.getCurrentPage
                ? $scope.getCurrentPage.toString() : '',
                data: {} };
            playerTelemetryUtilsService.navigateTelemetry(telemetryNavData);
            $scope.contentProgress
            = $scope.getCurrentPage * 100 / $scope.totalPageNumber;
        ...
        $scope.goToPage = function (pageNumber) {
            ...
            var telemetryData = {
                ...
                type: 'TOUCH',
                subtype: '',
                data: { stageId: $scope.getCurrentPage
                        ? $scope.getCurrentPage.toString() : ''
                }
            };
            playerTelemetryUtilsService.updateTelemetry(telemetryData);
