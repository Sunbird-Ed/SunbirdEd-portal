    ...
    $.fn.uploadFile = function (options) {
        ...
        function serializeAndUploadFiles(s, obj, files) {
            for (var i = 0; i < files.length; i++) {
                if (!isFileTypeAllowed(obj, s, files[i].type)) {
            	...
            	var fileMimeType = files[i].type;
            	if (fileMimeType.toLowerCase().indexOf('video') >= 0)
            		pd.filename.append('<br><i class="glyphicon glyphicon-facetime-video"></i>');
            	else if (fileMimeType.toLowerCase().indexOf('image') >= 0)
            		pd.filename.append('<br><i class="glyphicon glyphicon-picture"></i>');
            	else
            		pd.filename.append('<br><i class="glyphicon glyphicon-folder-open"></i>');
                pd.filename.append('<br>' + fileNameStr);
                var form = $("<form style='display:block; position:absolute;left: 150px;' class='" + obj.formGroup + "' method='" + s.method + "' action='" + s.url + "' enctype='" + s.enctype + "'></form>");
                form.appendTo('body');
                
                var fileObj = {};
            	fileObj.name = fileNameStr;
            	fileObj.type = fileMimeType;
            	var fileArray = [];
        ...
        function isFileTypeAllowed(obj, s, fileType) {
        	if (!fileType || fileType == '')
    			return false;
            var fileTypes = s.allowedTypes.toLowerCase().split(",");
            if (s.allowedTypes != "*") {
            	var valid = false;
            	for (var i=0; i<fileTypes.length; i++) {
            		var type = fileTypes[i];
            		var idx = fileType.toLowerCase().indexOf(type.toLowerCase());
            		if (idx >= 0) {
            			valid = true;
            		}
            	}
        ...
        function createCutomInputFile(obj, group, s, uploadLabel) {
            ...
            fileInput.change(function () {
                ...
                if (this.files) //support reading files
                {
                    ...
                    var fileType = document.getElementById(fileUploadId).files[0].type;
                    var flist = [];
                    var fileObj = {};
                    fileObj.name = filenameStr;
                    fileObj.type = fileType;
                    fileArray.push(fileObj);
                    if (!isFileTypeAllowed(obj, s, fileType)) {
                ...
                form.addClass(group);
                if (this.files) //use HTML5 support and split file submission
                {
                    ...
                    var fileList = "";
                    if(s.showFileCounter)
    		    		fileList += obj.fileCounter + s.fileCounterStyle + fileArray[0].name+"<br>";
        			else
	            		fileList += fileArray[0].name+"<br>";;
                    obj.fileCounter++;
                    var pd = new createProgressDiv(obj, s);
                    
                    var fileMimeType = fileArray[0].type;
                	if (fileMimeType.toLowerCase().indexOf('video') >= 0)
                		pd.filename.append('<br><i class="glyphicon glyphicon-facetime-video"></i>');
                	else if (fileMimeType.toLowerCase().indexOf('image') >= 0)
                		pd.filename.append('<br><i class="glyphicon glyphicon-picture"></i>');
                		...
                		pd.filename.append('<br><i class="glyphicon glyphicon-folder-open"></i>');
                    pd.filename.append('<br>' + fileList);
        ...
        function createProgressDiv(obj, s) {
            ...
            this.filename = $("<div class='ajax-file-upload-filename'><b>Media</b></div>").appendTo(this.statusbar);
            ...
            if (s.statusBarDiv) {
            	$('#' + s.statusBarDiv).append(this.statusbar);
            } else {
