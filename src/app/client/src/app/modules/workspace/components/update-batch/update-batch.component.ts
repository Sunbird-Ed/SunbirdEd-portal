...
export class UpdateBatchComponent implements OnInit, OnDestroy {
  ...
  */
  private getUserList(query: string = '', type) {
    ...
    this.batchService.getUserList(requestBody).takeUntil(this.unsubscribe)
      .subscribe((res) => {
  ...
  public updateBatch() {
    ...
    this.batchService.updateBatch(requestBody).takeUntil(this.unsubscribe)
      .subscribe((response) => {
  ...
  private updateParticipantsToBatch(batchId, participants) {
    ...
    this.batchService.addUsersToBatch(userRequest, batchId).takeUntil(this.unsubscribe)
      .subscribe((res) => {
        this.disableSubmitBtn = false;
        this.toasterService.success(this.resourceService.messages.smsg.m0033);
        this.reload();
