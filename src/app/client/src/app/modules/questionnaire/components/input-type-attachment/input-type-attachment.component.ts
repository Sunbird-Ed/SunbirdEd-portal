...
export class InputTypeAttachmentComponent implements OnInit {
  ...
  async onAddApproval(file) {
    ...
    let html = `
    ${this.resourceService.frmelmnts.alert.evidence_content_policy}<a href='https://diksha.gov.in/term-of-use.html' target="_blank">${this.resourceService.frmelmnts.alert.evidenceContentPolicyLabel}</a> .${this.resourceService.frmelmnts.alert.uploadevidencecontent}
    `;
