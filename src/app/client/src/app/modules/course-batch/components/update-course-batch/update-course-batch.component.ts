  ...
  deleteBatchDetails(user) {
    if (this.batchDetails && this.batchDetails.participant ||
      ...
      if (_.isArray(user)) {
        ...
        if (selectedItemId.length > 0) {
          _.forEach(selectedItemId, (selectedItem, value) => {
            ...
            if (participantIndex !== -1) {
              this.batchDetails.participant = _.omit(this.batchDetails.participant, selectedItem);
            }
          });
        } else {
        ...
        if (participantIndex !== -1) {
          this.batchDetails.participant = _.omit(this.batchDetails.participant, user.id);
        }
      }
