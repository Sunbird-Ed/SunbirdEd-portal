...
export class OnDemandReportService {
  ...
  getReportList(tag: string) {
    ...
    // return this.http.get(this.config.urlConFig.URLS.REPORT_PREFIX + this.config.urlConFig.URLS.REPORT.JOB_LIST + '/' + 'test-tag', options);
  ...
  getReport(tag: string, requestId: string) {
    const options = {headers: {'Content-Type': 'application/json'}};
    //return this.http.get(this.config.urlConFig.URLS.REPORT_PREFIX + this.config.urlConFig.URLS.REPORT.JOB_REQUEST_READ + '/' + 'test-tag/A09115FCBEC94CE6ACEB4D9BBFDBCBCF', options);
