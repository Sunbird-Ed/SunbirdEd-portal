export const MockResponse = {
    ServerResponse: {
        'code': 'a5392316-994b-456c-b14b-2d11316c843a',
        'subject': 'English',
        'language': [
            'English'
        ],
        'mimeType': 'text/x-url',
        'medium': 'Hindi',
        'createdOn': '2018-05-25T06:21:20.450+0000',
        'gradeLevel': [
            'Class 1'
        ],
        'appIcon': 'https://ekstep-public-dev.s3-ap-south-1.amazonaws.com/content/do_1125110622654464001294/ ' +
            'artifact/62f05664348aeff61aa195d0dc3caba5_1527228627157.thumb.jpg',
        'collections': [
            {
                'identifier': 'do_1125131911304970241310',
                'name': 'Untitled Course Unit',
                'objectType': 'Content',
                'relation': 'hasSequenceMember',
                'description': null,
                'index': null,
                'status': null,
                'depth': null,
                'mimeType': null,
                'visibility': null,
                'compatibilityLevel': null
            },
            {
                'identifier': 'do_1125110676518584321298',
                'name': 'Untitled Course Unit',
                'objectType': 'Content',
                'relation': 'hasSequenceMember',
                'description': null,
                'index': null,
                'status': null,
                'depth': null,
                'mimeType': null,
                'visibility': null,
                'compatibilityLevel': null
            },
            {
                'identifier': 'do_1125131881347563521308',
                'name': 'Untitled Course Unit',
                'objectType': 'Content',
                'relation': 'hasSequenceMember',
                'description': null,
                'index': null,
                'status': null,
                'depth': null,
                'mimeType': null,
                'visibility': null,
                'compatibilityLevel': null
            },
            {
                'identifier': 'do_1125131779825172481306',
                'name': 'Untitled Course Unit',
                'objectType': 'Content',
                'relation': 'hasSequenceMember',
                'description': null,
                'index': null,
                'status': null,
                'depth': null,
                'mimeType': null,
                'visibility': null,
                'compatibilityLevel': null
            },
            {
                'identifier': 'do_1125131748116070401304',
                'name': 'Testing',
                'objectType': 'Content',
                'relation': 'hasSequenceMember',
                'description': null,
                'index': null,
                'status': null,
                'depth': null,
                'mimeType': null,
                'visibility': null,
                'compatibilityLevel': null
            },
            {
                'identifier': 'do_112519655803256832177',
                'name': 'Collection Ext test',
                'objectType': 'Content',
                'relation': 'hasSequenceMember',
                'description': 'Untitled Collection',
                'index': null,
                'status': null,
                'depth': null,
                'mimeType': null,
                'visibility': null,
                'compatibilityLevel': null
            }
        ],
        'artifactUrl': 'http://www.dailymotion.com/video/x175su1',
        'lastUpdatedOn': '2018-05-31T11:23:13.620+0000',
        'contentType': 'Resource',
        'identifier': 'do_1125110622654464001294',
        'audience': [
            'Learner'
        ],
        'creator': 'sunbird Test',
        'visibility': 'Default',
        'mediaType': 'content',
        'osId': 'org.ekstep.quiz.app',
        'languageCode': 'en',
        'versionKey': '1527765793620',
        'framework': 'NCF',
        'concepts': [
            {
                'identifier': 'SC7',
                'name': 'Earth',
                'objectType': 'Concept',
                'relation': 'associatedTo',
                'description': 'Earth',
                'index': null,
                'status': null,
                'depth': null,
                'mimeType': null,
                'visibility': null,
                'compatibilityLevel': null
            }
        ],
        'createdBy': '63b0870c-f370-4f96-842d-f6a7fa2db1df',
        'name': 'A5 ext link',
        'board': 'CBSE',
        'status': 'Live',
        'resourceType': 'Practice'
    },
    resourceBundle: {
        'messages': {
            'imsg': {
                'm0034': 'As the content is from an external source, it will be opened in a new tab.'
            },
            'stmsg': { 'm0009': 'Unable to play, please try again or close' }
        }
    }
};
