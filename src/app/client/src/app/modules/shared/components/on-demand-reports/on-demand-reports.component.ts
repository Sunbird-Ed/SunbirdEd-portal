...
export class OnDemandReportsComponent implements OnInit {
  ...
  ngOnInit() {
    this.onDemandReportService.getReportList('test').subscribe((data) => {
      this.onDemandReportData = data['result']['jobs'];
    });
    this.columns = [
  ...
  getReport(){
    this.onDemandReportService.getReport('test', 'test').subscribe((data) => {
