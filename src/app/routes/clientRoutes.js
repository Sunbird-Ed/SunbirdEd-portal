...
module.exports = (app, keycloak) => {
  ...
  '/resources/*', '/myActivity', '/myActivity/*', '/org/*', '/manage', '/contribute','/contribute/*','/groups','/groups/*', '/my-groups','/my-groups/*','/certs/configure/*',
   '/observation', '/observation/*','/solution','/solution/*','/questionnaire','/questionnaire/*', '/uci-admin', '/uci-admin/*','/explore-events/*'], 
  session({
  ...
  app.all(['/', '/get', '/:slug/get', '/:slug/get/dial/:dialCode',  '/get/dial/:dialCode', '/explore',
    ...
    '/sign-in/*', '/download/*', '/accountMerge/*','/:slug/accountMerge/*', '/:slug/download/*', '/certs/*', '/:slug/certs/*', '/recover/*', '/:slug/recover/*', '/explore-groups','/explore-events/*',
    '/guest-profile'], 
    session({
      secret: envHelper.PORTAL_SESSION_SECRET_KEY,
      resave: false,
...
const redirectTologgedInPage = (req, res) => {
	let redirectRoutes = { '/explore': '/resources', '/explore/1': '/search/Library/1', '/explore-course': '/learn', '/explore-course/1': '/search/Courses/1', '/guest-profile': '/profile', '/explore-groups': '/my-groups' ,'/explore-events':'/explore-events'};
	if (req.params.slug) {
		redirectRoutes = {
