...
module.exports = function (app) {
  ...
  app.patch('/learner/portal/user/v1/update',
    ...
    proxy(envHelper.learner_Service_Local_BaseUrl, {
      ...
      userResDecorator: (proxyRes, proxyResData, req, res) => {
        try {
            logDebug(req, err, {msg: '/learner/portal/user/v1/update called'});
  ...
  app.post('/learner/content/v1/media/upload',
    ...
    proxy(learnerURL, {
      ...
      userResDecorator: function (proxyRes, proxyResData,  req, res) {
        try {
          logInfo(req, {}, '/learner/content/v1/media/upload called');
  ...
  app.all('/learner/data/v1/role/read',
    ...
    proxy(learnerURL, {
      ...
      userResDecorator: function (proxyRes, proxyResData,  req, res) {
        try {
          logInfo(req, {}, `/learner/data/v1/role/read called`)
          let data = JSON.parse(proxyResData.toString('utf8'))
          if(req.method === 'GET' && proxyRes.statusCode === 404 && (typeof data.message === 'string' && data.message.toLowerCase() === 'API not found with these values'.toLowerCase())) res.redirect('/')
          else return proxyUtils.handleSessionExpiry(proxyRes, proxyResData, req, res, data);
        } catch (err) {
  ...
  app.all('/learner/*',
    ...
    proxy(learnerURL, {
      ...
      userResDecorator: (proxyRes, proxyResData, req, res) => {
        try {
            logInfo(req, {}, {msg: 'learner/* userResDecorator'});
            const data = JSON.parse(proxyResData.toString('utf8'));
            if(req.method === 'GET' && proxyRes.statusCode === 404 && (typeof data.message === 'string' && data.message.toLowerCase() === 'API not found with these values'.toLowerCase())) res.redirect('/')
            else return proxyUtils.handleSessionExpiry(proxyRes, proxyResData, req, res, data);
        } catch(err) {
