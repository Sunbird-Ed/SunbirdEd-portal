...
angular.module('playerApp')
  .service('announcementService', ['httpServiceJava', 'config', '$http', function(httpServiceJava, config, $http) {
     ...
     * @instance
     */
    this.httpCall = function httpCall(url, data, method, header) {
      return $http({
        method: method,
        url: url,
        headers: header,
    ...
    this.getOutBoxAnnouncementList = function() {
      ...
      var URL = 'http://localhost:3000/api/announcement/v1/user/outbox'
      var data = {
        "request": {
          "userId": "1231-12313-132123-12313",
          "limit": 10,
          "status": "read"
        }
      };

      //return httpServiceJava.post(URL, data, headers);
      return this.httpCall(URL, data, 'POST', headers);
    }
