	...
	function($rootScope, $scope, $state, $stateParams, $timeout, config, toasterService, fileUpload, AnnouncementModel, announcementAdapter) {
		...
		composeAnn.init = function() {
			...
			if (composeAnn.stepNumber === 1) {
				announcementAdapter.getDefinitions($rootScope.rootOrgId).then(function(response) {
		...
		composeAnn.enableRecepientBtn = function(status) {
			...
			if (composeAnn.announcement && composeAnn.announcement.details.title && composeAnn.announcement.details.from && (true || composeAnn.announcement.details.type) && (composeAnn.uploadAttchement || composeAnn.announcement.details.description || links.length)) {
		...
		composeAnn.confirmRecipients = function() {
			...
			if (composeAnn.announcement.selTar && composeAnn.announcement.selTar.length === 0) {
				...
				$state.go('announcementResend', {
					stepNumber: 2,
					announcement: composeAnn.announcement,
					isMetaModifiedSteps: composeAnn.isMetaModifiedSteps
				}, {
					reload: true
				})
				return false
		...
		window.removeResendAnnAttachment = function(item, pos) {
		...
		composeAnn.initializeFileUploader = function(resend) {
			...
			if (resend) {
				fileUpload.createFineUploadInstance(options, function(data) {
					...
					var attachments = angular.copy(composeAnn.announcement.attachments)
					if ($('#old-file-list').length <= 0) {
						...
						_.forEach(attachments, function(attachment, pos) {
							$('#old-file-list').append('<li class="qq-upload-retryable w3-container' +
								' w3-border w3-round-xlarge qq-upload-success"><i onclick="removeResendAnnAttachment(this,' + pos + ')" class="remove icon cursor-pointer" style="float:right;"></i><span class="qq-upload-file-selector qq-upload-file"' +
								' style="margin-top: -30px !important;width: 222px;">' + attachment.name + '</span></li>')
						})
					}
				})
			}
