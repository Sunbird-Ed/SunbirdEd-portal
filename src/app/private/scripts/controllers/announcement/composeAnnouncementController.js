'use strict'
angular.module('playerApp').controller('composeAnnouncementCtrl', ['$rootScope', '$scope', '$state', '$stateParams', '$timeout', 'config', 'toasterService', 'announcementService', 'fileUpload', 'AnnouncementModel', 'announcementAdapter', 'portalTelemetryService',
				...
				if ($('#old-file-list').length <= 0) {
					...
					_.forEach(attachments, function(attachment, pos) {
						$('#old-file-list').append('<li class="qq-upload-retryable w3-container' +
							' w3-border w3-round-xlarge qq-upload-success"><i onclick="removeCreateAnnAttachment(this,'+pos+')" class="remove icon cursor-pointer" style="float:right;"></i><span class="qq-upload-file-selector qq-upload-file"' +
