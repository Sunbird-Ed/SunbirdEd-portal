    ...
    function($rootScope, $scope, $state, $stateParams, $timeout, config, toasterService, fileUpload,
        ...
        composeAnn.initializeFileUploader = function(resend) {
            ...
            fileUpload.createFineUploadInstance(options, function(data) {
                ...
                if ($('#old-file-list').length <= 0) {
                    $('.qq-upload-list').parent().prepend(
                        '<ul id="old-file-list" class="' + ' qq-upload-list"></ul>')
                    _.forEach(attachments, function(attachment, pos) {
                        $('#old-file-list').append(
                            '<li class="qq-upload-retryable w3-container' +
                            ' w3-border w3-round-xlarge qq-upload-success"><i id="removeFile" onclick="removeCreateAnnAttachment(this,' +
                            pos +
                            ')" class="remove icon cursor-pointer" style="float:right;"></i><span class="qq-upload-file-selector qq-upload-file"' +
                            ' style="margin-top: -30px !important;width: 222px;">' +
        ...
        composeAnn.init = function() {
            ...
            if (composeAnn.stepNumber === 1 && composeAnn.announcement === null) {
                if (composeAnn.editAction) {
                    composeAnn.announcement = composeAnn.getResend($stateParams.announcementId)
                } else {
        ...
        composeAnn.getDefinitions = function(rootOrgId) {
            announcementAdapter.getDefinitions(rootOrgId).then(function(response) {
                ...
                return composeAnn.announcement
            }, function(err) {
