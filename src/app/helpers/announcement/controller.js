...
class AnnouncementController {
    ...
    __getAnnouncementById(requestObj) {
        return new Promise((resolve, reject) => {
            ...
            this.objectStoreRest.findObject(query)
                ...
                .catch((error) => {
                    reject(new AppError({
                        message: error.message || 'Unable to find',
                        status: error.status || HttpStatus.INTERNAL_SERVER_ERROR
                    }))
                })
        })
            ...
            return new Promise((resolve, reject) => {
                if (status) {
                    this.objectStoreRest.updateObjectById(query)
                        ...
                        .catch((error) => {
                            reject(new AppError({
                                message: error.message || 'Unable to cancel!',
                                status: error.status || HttpStatus.INTERNAL_SERVER_ERROR
                            }))
                        })
                } else {
    ...
    __getUserInbox() {
        return async((requestObj) => {
            ...
            let userProfile = await (this.__getUserProfile({
    ...
    getUserOutbox(requestObj) {
        return new Promise((resolve, reject) => {
            ...
            if (!request.isValid) throw 'Unable to fetch'
    ...
    getResend() {
        return async((requestObj) => {
            try {
                ...
                throw new AppError({
                    message: error.message || 'Unable to resend',
                    status: error.status || HttpStatus.INTERNAL_SERVER_ERROR
                })
            }
        })
