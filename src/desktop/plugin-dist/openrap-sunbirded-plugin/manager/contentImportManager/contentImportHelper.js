var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
const IContentImport_1 = require("./IContentImport");
const StreamZip = require("node-stream-zip");
const fs = require("fs");
const _ = require("lodash");
const path = require("path");
const manifest_1 = require("../../manifest");
const api_1 = require("OpenRAP/dist/api");
const config_1 = require("../../config");
const rxjs_1 = require("rxjs");
const operators_1 = require("rxjs/operators");
const collectionMimeType = "application/vnd.ekstep.content-collection";
let zipHandler;
let zipEntries;
let dbContents;
let manifestJson;
let contentImportData;
const fileSDK = api_1.containerAPI.getFileSDKInstance(manifest_1.manifest.id);
let fileSDKContentInstance = api_1.containerAPI.getFileSDKInstance(manifest_1.manifest.id);
const pluginBasePath = fileSDK.getAbsPath("");
let contentFolder = fileSDK.getAbsPath("content");
const ecarFolder = fileSDK.getAbsPath("ecars");
const systemSDK = api_1.containerAPI.getSystemSDKInstance(manifest_1.manifest.id);
let availableDiskSpace;
const syncCloser = (initialProgress, percentage, totalSize = contentImportData.metaData.contentSize) => {
    initialProgress = initialProgress ? initialProgress : contentImportData.progress;
    let completed = 1;
    const syncData$ = new rxjs_1.Subject();
    const subscription = syncData$.pipe(operators_1.throttleTime(2500)).subscribe((data) => {
        const newProgress = ((completed / totalSize) * percentage);
        contentImportData.progress = initialProgress + newProgress;
        sendMessage("DATA_SYNC");
    });
    return (chunk = 0) => {
        completed += chunk;
        syncData$.next(completed);
        return subscription;
    };
};
const copyEcar = () => __awaiter(this, void 0, void 0, function* () {
    try {
        process.send({ message: "LOG", logType: "info",
            logBody: [contentImportData._id, "Disk Space availability check",
                contentImportData.metaData.contentSize, availableDiskSpace] });
        if (contentImportData.metaData.contentSize > availableDiskSpace) {
            throw IContentImport_1.getErrorObj({ message: "Disk space is low, couldn't copy Ecar" }, "LOW_DISK_SPACE");
        }
        process.send({ message: "LOG", logType: "info", logBody: [contentImportData._id, "copping ecar from src location to ecar folder", contentImportData.metaData.ecarSourcePath, ecarFolder] });
        const syncFunc = syncCloser(IContentImport_1.ImportProgress.COPY_ECAR, 25);
        const toStream = fs.createWriteStream(path.join(ecarFolder, contentImportData._id + ".ecar"));
        const fromStream = fs.createReadStream(contentImportData.metaData.ecarSourcePath);
        fromStream.pipe(toStream);
        fromStream.on("data", (buffer) => syncFunc(buffer.length));
        toStream.on("finish", (data) => {
            syncFunc().unsubscribe();
            contentImportData.progress = IContentImport_1.ImportProgress.PARSE_ECAR;
            sendMessage(IContentImport_1.ImportSteps.copyEcar);
        });
        toStream.on("error", (err) => {
            syncFunc().unsubscribe();
            sendMessage("IMPORT_ERROR", IContentImport_1.getErrorObj(err, "UNHANDLED_COPY_ECAR_ERROR"));
        });
        fromStream.on("error", (err) => {
            syncFunc().unsubscribe();
            sendMessage("IMPORT_ERROR", IContentImport_1.getErrorObj(err, "UNHANDLED_COPY_ECAR_ERROR"));
        });
    }
    catch (err) {
        sendMessage("IMPORT_ERROR", IContentImport_1.getErrorObj(err, "UNHANDLED_COPY_ECAR_ERROR"));
    }
});
const parseEcar = () => __awaiter(this, void 0, void 0, function* () {
    try {
        const ecarBasePath = path.join(ecarFolder, contentImportData._id + ".ecar");
        const contentBasePath = path.join(contentFolder, contentImportData._id); // temp path
        zipHandler = yield loadZipHandler(ecarBasePath).catch(IContentImport_1.handelError("LOAD_ECAR"));
        yield fileSDKContentInstance.mkdir(contentImportData._id);
        zipEntries = zipHandler.entries();
        const manifestEntry = zipEntries["manifest.json"] || zipEntries["/manifest.json"];
        if (!manifestEntry) {
            throw IContentImport_1.getErrorObj({ message: "manifest.json is missing in ecar" }, "MANIFEST_MISSING");
        }
        yield new Promise((res, rej) => zipHandler.extract(manifestEntry.name, contentBasePath, (err) => err ? rej(err) : res()));
        manifestJson = yield fileSDK.readJSON(path.join(contentBasePath, "manifest.json"));
        const parent = _.get(manifestJson, "archive.items[0]");
        if (_.get(parent, "visibility") !== "Default") {
            throw IContentImport_1.getErrorObj({ message: `manifest.json dosn"t contain content with visibility Default` }, "INVALID_MANIFEST");
        }
        if (parent.compatibilityLevel > config_1.default.get("CONTENT_COMPATIBILITY_LEVEL")) {
            throw IContentImport_1.getErrorObj({ message: `${parent.compatibilityLevel} not supported. Required ${config_1.default.get("CONTENT_COMPATIBILITY_LEVEL")} and below` }, "UNSUPPORTED_COMPATIBILITY_LEVEL");
        }
        contentImportData.metaData.contentId = parent.identifier;
        contentImportData.metaData.mimeType = parent.mimeType;
        contentImportData.metaData.contentType = parent.contentType;
        contentImportData.metaData.pkgVersion = _.toString(parent.pkgVersion) || "1.0";
        if (contentImportData.metaData.mimeType === "application/vnd.ekstep.content-collection") {
            contentImportData.metaData.childNodes = _.filter(_.get(manifestJson, "archive.items"), (item) => (item.mimeType !== "application/vnd.ekstep.content-collection"))
                .map((item) => item.identifier);
        }
        contentImportData.progress = IContentImport_1.ImportProgress.EXTRACT_ECAR;
        sendMessage(IContentImport_1.ImportSteps.parseEcar);
    }
    catch (err) {
        zipHandler.close();
        sendMessage("IMPORT_ERROR", IContentImport_1.getErrorObj(err, "UNHANDLED_PARSE_ECAR_ERROR"));
    }
});
const extractZipEntry = (identifier, contentBasePath, entry, syncFunc) => __awaiter(this, void 0, void 0, function* () {
    const zipEntry = (identifier === contentImportData.metaData.contentId) ? entry : identifier + "/" + entry;
    const entryObj = zipEntries[zipEntry] || zipEntries["/" + zipEntry] ||
        (!_.includes(["manifest.json", "hierarchy.json"], entry) && zipEntries["/" + entry]); // last checks to support mobile
    if (!entryObj) {
        return false;
    }
    if (!contentImportData.metaData.extractedEcarEntries[entryObj.name]) {
        yield new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            return zipHandler.extract(entryObj.name, path.join(contentFolder, ...contentBasePath), (err) => err ? reject(err) : resolve());
        }));
        contentImportData.metaData.extractedEcarEntries[entryObj.name] = true;
    }
    syncFunc(entryObj.compressedSize);
    return entryObj;
});
const checkSpaceAvailability = (entries, extractedEntries = contentImportData.metaData.extractedEcarEntries) => __awaiter(this, void 0, void 0, function* () {
    let contentSize = 0; // size in bytes
    for (const entry of _.values(entries)) {
        if (!extractedEntries[entry.name]) {
            contentSize += entry.size;
        }
    }
    process.send({ message: "LOG", logType: "info", logBody: [contentImportData._id, "Disk Space availability check",
            contentSize, availableDiskSpace] });
    if (contentSize > availableDiskSpace) { // bytes
        throw IContentImport_1.getErrorObj({ message: "Disk space is low, couldn't extract Ecar" }, "LOW_DISK_SPACE");
    }
});
const extractEcar = () => __awaiter(this, void 0, void 0, function* () {
    try {
        contentImportData.metaData.contentSkipped = [];
        contentImportData.metaData.contentAdded = [];
        zipHandler = zipHandler ||
            (yield loadZipHandler(path.join(ecarFolder, contentImportData._id + ".ecar")).catch(IContentImport_1.handelError("LOAD_ECAR")));
        manifestJson = manifestJson || (yield fileSDK.readJSON(path.join(contentFolder, contentImportData._id, "manifest.json")));
        zipEntries = zipHandler.entries();
        yield checkSpaceAvailability(zipEntries);
        const syncFunc = syncCloser(IContentImport_1.ImportProgress.EXTRACT_ECAR, 65);
        const artifactToBeUnzipped = [];
        let artifactToBeUnzippedSize = 0;
        const extractContent = (content, parent, collection) => __awaiter(this, void 0, void 0, function* () {
            if (collection && !parent && !content.appIcon) { // exit if content is not leaf node or parent
                return;
            }
            const contentBasePath = (collection && !parent) ?
                [contentImportData.metaData.contentId, content.identifier] : [content.identifier];
            yield fileSDKContentInstance.mkdir(path.join(...contentBasePath));
            yield extractZipEntry(content.identifier, contentBasePath, content.appIcon, syncFunc);
            yield extractZipEntry(content.identifier, contentBasePath, "manifest.json", syncFunc);
            if (collection) {
                if (parent) { // extract hierarchy file for parent if its a collection
                    yield extractZipEntry(content.identifier, contentBasePath, "hierarchy.json", syncFunc);
                }
                return; // exit if content is collection
            }
            if (content.artifactUrl && content.contentDisposition === "online") { // exit if online only content
                contentImportData.metaData.contentAdded.push(content.identifier);
                return;
            }
            const artifactExtracted = yield extractZipEntry(content.identifier, contentBasePath, content.artifactUrl, syncFunc);
            if (!artifactExtracted) { // exit if artifact is missing
                contentImportData.metaData.contentSkipped.push({ id: content.identifier, reason: "ARTIFACT_MISSING" });
                return;
            }
            if (artifactExtracted.name.endsWith(".zip")) { // append zip entry along with size to unzip later
                artifactToBeUnzippedSize += artifactExtracted.compressedSize;
                artifactToBeUnzipped.push({
                    contentId: content.identifier,
                    src: path.join(...contentBasePath, path.basename(artifactExtracted.name)),
                    size: artifactExtracted.compressedSize,
                });
            }
            else {
                contentImportData.metaData.contentAdded.push(content.identifier);
            }
        });
        for (const content of _.get(manifestJson, "archive.items")) { // loop items in manifest and extract its contents
            const dbContent = _.find(dbContents, { identifier: content.identifier });
            if (!dbContent || content.pkgVersion > dbContent.pkgVersion ||
                !_.get(dbContent, "desktopAppMetadata.isAvailable")) { // if content not imported or new ver available
                yield extractContent(content, (content.identifier === contentImportData.metaData.contentId), (content.mimeType === collectionMimeType));
            }
            else {
                contentImportData.metaData.contentSkipped.push({ id: content.identifier, reason: "CONTENT_IMPORTED_ALREADY" });
            }
        }
        syncFunc().unsubscribe();
        process.send({ message: "LOG", logType: "info", logBody: [contentImportData._id, "ecar extracted"] });
        yield unzipArtifacts(artifactToBeUnzipped, artifactToBeUnzippedSize)
            .catch(IContentImport_1.handelError("EXTRACT_ARTIFACTS")); // extract all zip artifacts
        process.send({ message: "LOG", logType: "info", logBody: [contentImportData._id, "artifacts unzipped"] });
        fileSDK.remove(path.join("ecars", contentImportData._id + ".ecar"))
            .catch((err) => process.send({ message: "LOG", logType: "error", logBody: [contentImportData._id, "error while deleting ecar folder", location] }));
        yield removeFile(contentImportData._id); // delete temp content folder which has manifest.json
        sendMessage(IContentImport_1.ImportSteps.extractEcar);
    }
    catch (err) {
        zipHandler.close();
        sendMessage("IMPORT_ERROR", IContentImport_1.getErrorObj(err, "UNHANDLED_EXTRACT_ECAR_ERROR"));
    }
});
const removeFile = (location) => {
    return fileSDKContentInstance.remove(location)
        .catch((err) => process.send({ message: "LOG", logType: "error", logBody: [contentImportData._id, "error while deleting ecar folder", location] }));
};
const unzipFile = (src, dest = path.dirname(src)) => __awaiter(this, void 0, void 0, function* () {
    yield fileSDKContentInstance.unzip(src, dest, false)
        .catch((err) => process.send({ message: "LOG", logType: "error", logBody: [contentImportData._id, "error while unzip file", src] }));
});
const unzipArtifacts = (artifactToBeUnzipped = [], artifactToBeUnzippedSize) => __awaiter(this, void 0, void 0, function* () {
    const syncFunc = syncCloser(IContentImport_1.ImportProgress.EXTRACT_ARTIFACT, 9, artifactToBeUnzippedSize);
    for (const artifact of artifactToBeUnzipped) {
        syncFunc(artifact.size);
        if (!contentImportData.metaData.artifactUnzipped[artifact.src]) {
            yield extractZipArtifact(artifact).catch((err) => {
                contentImportData.metaData.contentSkipped.push({ id: artifact.contentId, reason: "UNZIP_ARTIFACT_ERROR" });
            });
            contentImportData.metaData.artifactUnzipped[artifact.src] = true;
        }
    }
    syncFunc().unsubscribe();
});
const extractZipArtifact = (artifact) => __awaiter(this, void 0, void 0, function* () {
    const zipArtifactZipHandler = yield loadZipHandler(path.join(contentFolder, artifact.src));
    const zipArtifactEntries = zipArtifactZipHandler.entries();
    yield checkSpaceAvailability(zipArtifactEntries);
    yield unzipFile(artifact.src);
    yield removeFile(artifact.src);
    contentImportData.metaData.contentAdded.push(artifact.contentId);
    zipArtifactZipHandler.close();
});
const loadZipHandler = (filePath) => __awaiter(this, void 0, void 0, function* () {
    const zip = new StreamZip({ file: filePath, storeEntries: true, skipEntryNameValidation: true });
    return new Promise((resolve, reject) => {
        zip.on("ready", () => resolve(zip));
        zip.on("error", reject);
    });
});
const sendMessage = (message, err) => {
    const messageObj = {
        message, contentImportData,
    };
    if (err) {
        messageObj.err = err;
    }
    process.send(messageObj);
};
process.on("message", (data) => {
    if (_.includes([IContentImport_1.ImportSteps.copyEcar, IContentImport_1.ImportSteps.parseEcar, IContentImport_1.ImportSteps.extractEcar], data.message)) {
        contentImportData = data.contentImportData;
        if (data.contentFolder) {
            contentFolder = data.contentFolder;
            fileSDKContentInstance = api_1.containerAPI.getFileSDKInstance(manifest_1.manifest.id, contentFolder);
        }
        if (data.availableDiskSpace) {
            availableDiskSpace = data.availableDiskSpace;
        }
    }
    if (data.message === IContentImport_1.ImportSteps.copyEcar) {
        copyEcar();
    }
    else if (data.message === IContentImport_1.ImportSteps.parseEcar) {
        parseEcar();
    }
    else if (data.message === IContentImport_1.ImportSteps.extractEcar) {
        dbContents = data.dbContents;
        extractEcar();
    }
    else if (data.message === "KILL") {
        sendMessage("DATA_SYNC_KILL");
    }
});
